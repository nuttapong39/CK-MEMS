...
import {
    PlusIcon, MagnifyingGlassIcon, FunnelIcon, ChevronLeftIcon, ChevronRightIcon,
...
watch(() => [filters.department_id, filters.status, filters.per_page], () => {
    filters.page = 1;
    load();
});
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-0.5">
                    ทั้งหมด {{ meta.total }} รายการ ใน {{ auth.hospitalName }}
                </p>
            </div>
            <RouterLink
                :to="{ name: 'equipment.create' }"
                v-if="auth.hasAnyRole(['admin', 'staff'])"
                class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold shadow-lg shadow-blue-500/30 hover:shadow-xl transition self-start"
            >
        ...
        <div class="card-base overflow-hidden">
                ...
                <table class="w-full text-sm">
                    <thead class="bg-slate-50 border-b border-slate-200">
                        <tr class="text-left text-xs uppercase tracking-wider text-slate-500">
                            <th class="px-5 py-3 font-semibold">ID Code</th>
                            <th class="px-5 py-3 font-semibold">ชื่อเครื่องมือ</th>
                            <th class="px-5 py-3 font-semibold">หน่วยงาน</th>
                            <th class="px-5 py-3 font-semibold">ยี่ห้อ / รุ่น</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <tr v-if="loading">
                            <td colspan="6" class="px-5 py-12 text-center text-sm text-slate-400">กำลังโหลด...</td>
                        </tr>
                        ...
                        <tr v-for="item in items" :key="item.id" class="hover:bg-slate-50 transition-colors">
                            <td class="px-5 py-3 font-mono text-xs font-semibold text-blue-700">{{ item.id_code }}</td>
                            <td class="px-5 py-3">
                                <div class="font-medium text-slate-800">{{ item.name_th }}</div>
                                <div v-if="item.name_en" class="text-xs text-slate-500">{{ item.name_en }}</div>
                            </td>
                            <td class="px-5 py-3">
                            ...
                            <td class="px-5 py-3 text-right">
                                <div class="inline-flex gap-1">
                                    <button class="p-1.5 rounded-lg hover:bg-blue-50 text-blue-600 transition"><EyeIcon class="w-4 h-4" /></button>
                                    <button v-if="auth.hasAnyRole(['admin','staff'])" class="p-1.5 rounded-lg hover:bg-amber-50 text-amber-600 transition"><PencilSquareIcon class="w-4 h-4" /></button>
                                    <button v-if="auth.isAdmin" @click="handleDelete(item)" class="p-1.5 rounded-lg hover:bg-rose-50 text-rose-600 transition"><TrashIcon class="w-4 h-4" /></button>
                                </div>
                            </td>
                        </tr>
                    </tbody>
            ...
            <div v-if="meta.last_page > 1" class="flex items-center justify-between px-5 py-4 border-t border-slate-100">
                <div class="text-xs text-slate-500">
                    หน้า {{ meta.current_page }} จาก {{ meta.last_page }} ({{ meta.total }} รายการ)
                </div>
                <div class="flex items-center gap-1">
                    <button :disabled="filters.page === 1" @click="filters.page--" class="p-2 rounded-lg border border-slate-200 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed">
                    ...
                    >{{ p }}</button>
                    <button :disabled="filters.page === meta.last_page" @click="filters.page++" class="p-2 rounded-lg border border-slate-200 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed">
