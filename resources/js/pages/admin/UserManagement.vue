...
async function resetPassword(user) {
    const { value: pwd } = await Swal.fire({
        ...
        input: 'password',
        inputLabel: 'รหัสผ่านใหม่ (≥6 ตัวอักษร)',
        inputAttributes: { autocapitalize: 'off', autocorrect: 'off', minlength: 6 },
        showCancelButton: true,
        confirmButtonText: 'บันทึก', cancelButtonText: 'ยกเลิก',
        confirmButtonColor: '#2563eb',
        inputValidator: (v) => (!v || v.length < 6) ? 'อย่างน้อย 6 ตัวอักษร' : null,
    });
    if (!pwd) return;
    try {
        await usersApi.resetPassword(user.id, pwd);
        Swal.fire({ icon: 'success', title: 'รีเซ็ตเรียบร้อย', timer: 1200, showConfirmButton: false });
    } catch (e) {
    ...
    <div class="space-y-5">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <button @click="openCreate"
                class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-rose-500 to-orange-500 text-white font-semibold shadow-lg shadow-rose-500/30 hover:shadow-xl transition self-start">
                <PlusIcon class="w-5 h-5" /> เพิ่มผู้ใช้
            </button>
        </div>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-slate-50 border-b border-slate-200">
                        <tr class="text-left text-xs uppercase tracking-wider text-slate-500">
                            <th class="px-5 py-3 font-semibold">ชื่อ-นามสกุล</th>
                            <th class="px-5 py-3 font-semibold">รหัส</th>
                            <th class="px-5 py-3 font-semibold">Email</th>
                            <th class="px-5 py-3 font-semibold">หน่วยงาน</th>
                            <th class="px-5 py-3 font-semibold">สิทธิ์</th>
                            <th class="px-5 py-3 font-semibold">สถานะ</th>
                            <th class="px-5 py-3 font-semibold text-right">จัดการ</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                        <tr v-if="loading"><td colspan="7" class="px-5 py-12 text-center text-slate-400">กำลังโหลด...</td></tr>
                        <tr v-else-if="!items.length"><td colspan="7" class="px-5 py-12 text-center text-slate-400">ไม่พบผู้ใช้</td></tr>
                        <tr v-for="u in items" :key="u.id" class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-5 py-3 text-xs text-slate-600 font-mono">{{ u.employee_code || '—' }}</td>
                            <td class="px-5 py-3 text-xs text-slate-600">{{ u.email }}</td>
                            <td class="px-5 py-3 text-xs text-slate-600">{{ u.department?.name_th || '—' }}</td>
                            <td class="px-5 py-3">
                                <template v-for="r in u.roles" :key="r.id || r">
                                    <span :class="['text-xs px-2 py-1 rounded-full font-medium', roleBadge(r.name || r)]">
                            ...
                            <td class="px-5 py-3 text-right">
                                <div class="inline-flex gap-1">
                                    <button @click="openEdit(u)" class="p-1.5 rounded-lg hover:bg-amber-50 text-amber-600" title="แก้ไข">
                                        <PencilSquareIcon class="w-4 h-4" />
                                    </button>
                                    <button @click="resetPassword(u)" class="p-1.5 rounded-lg hover:bg-blue-50 text-blue-600" title="รีเซ็ตรหัสผ่าน">
                                        <KeyIcon class="w-4 h-4" />
                                    </button>
                                    <button @click="deleteUser(u)" :disabled="u.id === auth.user?.id"
                                        class="p-1.5 rounded-lg hover:bg-rose-50 text-rose-600 disabled:opacity-30 disabled:cursor-not-allowed" title="ลบ">
            ...
            <div v-if="meta.last_page > 1" class="flex items-center justify-between px-5 py-4 border-t border-slate-100">
                ...
                <div class="flex items-center gap-1">
                    ...
                    <button v-for="p in pageRange" :key="p" @click="filters.page = p"
                        :class="['min-w-[32px] h-8 px-2 rounded-lg text-sm font-medium', p === meta.current_page ? 'bg-blue-600 text-white' : 'border border-slate-200']">{{ p }}</button>
